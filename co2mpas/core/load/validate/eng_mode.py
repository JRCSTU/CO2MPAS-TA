...
class Constants(co2_utl.Constants):
...
def _select_declaration_data(data, diff=None):
    res = {}
    for k, v in sh.stack_nested_keys(dfl.DECLARATION_DATA):
...
def _extract_declaration_data(inputs, errors):
    diff = set()
    inputs = _select_declaration_data(inputs, diff)
    errors = _select_declaration_data(errors)
...
def eng_mode_parser(
        type_approval_mode, engineering_mode, soft_validation, inputs, errors):
    """
    Parse input data for the engineering mode.
    ...
    if type_approval_mode or not engineering_mode:
